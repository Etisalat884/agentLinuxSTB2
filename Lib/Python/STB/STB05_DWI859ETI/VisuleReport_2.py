...
def _parse_robot_timestamp(ts: str) -> datetime.datetime:
    """Parse Robot Framework timestamps with or without space."""
    ts = ts.strip()
    fmts = ["%Y%m%d %H:%M:%S.%f", "%Y%m%d%H:%M:%S.%f", "%Y-%m-%d %H:%M:%S.%f", "%Y-%m-%d%H:%M:%S.%f"]
    for fmt in fmts:
        try:
            return datetime.datetime.strptime(ts, fmt)
        except ValueError:
            continue
    # Fallback: try slicing milliseconds if wrong length
    if len(ts) > 3 and ts[-4] == '.':
        try:
            return datetime.datetime.strptime(ts[:-4], "%Y%m%d%H%M%S")
        except ValueError:
            pass
...
class TestReportGenerator:
    def __init__(self, trigger_id, agent_id, report_title="Test Report"):
        # self.xml_file = r"C:\Users\40040556\PycharmProjects\LaunchAgent\xmlReports"
        # self.VisuleReportpath = r"C:\Users\40040556\PycharmProjects\LaunchAgent\visualReports"
        # self.company_logo = r"C:\Users\40040556\PycharmProjects\LaunchAgent\EvQUAL_Logo.png"
        self.xml_file = r"/home/ltts/Documents/Visule_Report"
        ...
        self.api_url = "http://43.205.56.231:5000/update_data_dashboard?key="
        self.link = None
    ...
    def parse_robot_framework_xml(self):
        """
        ...
        for suite in root.iter('suite'):  # Use iter to find all nested suites
            ...
            for test in suite.findall('test'):  #
                ...
                # Only consider the last <status> tag (just before </test>)
                status_elements = test.findall('status')  #
                status_element = status_elements[-1] if status_elements else None  #

                status = "FAIL"
                elapsed_time_seconds = 0.0

                if status_element is not None:  #
                    raw_status = status_element.get('status', 'FAIL')  #
                    status = "PASS" if raw_status == "PASS" else "FAIL"  #

                    # Try direct elapsed attribute first
                    elapsed_attr = status_element.get('elapsed')  #
                    if elapsed_attr:
                        try:
                            elapsed_time_seconds = float(elapsed_attr)  #
                        except ValueError:
                            elapsed_time_seconds = 0.0
                    else:
                        # Compute from start/end time attributes if present
                        stime = status_element.get('starttime') or status_element.get('start_time')  #
                        etime = status_element.get('endtime') or status_element.get('end_time')  #
                        if stime and etime:
                            try:
                                dt_start = _parse_robot_timestamp(stime)  #
                                dt_end = _parse_robot_timestamp(etime)  #
                                elapsed_time_seconds = (dt_end - dt_start).total_seconds()  #
                            except Exception:
                                elapsed_time_seconds = 0.0
                ...
                data.append({  #
                    ...
                    'Duration (s)': elapsed_time_seconds,  #
                    'Module': module_name  #
                })
    ...
    def generate_html_report(self):
        ...
        self.parse_robot_framework_xml()
        print("line 253")
        self.calculate_kpis()
        print("line 255")
            ...
            <body>
                <div class='container'>
                    ...
                    <div class='charts-container'>
                        <div class='chart-row'>
                            <div class='chart' id='chart-pie'>{fig_pie.to_html(full_html=False, include_plotlyjs=False, div_id='pie-chart', config=self.chart_config_no_click)}</div>
                            <div class='chart' id='chart-donut'>{fig_donut.to_html(full_html=False, include_plotlyjs=False, div_id='donut-chart', config=self.chart_config_no_click)}</div>
                            <div class='chart' id='chart-stack'>{fig_stack.to_html(full_html=False, include_plotlyjs=False, div_id='stack-chart', config=self.chart_config_no_click)}</div>
                        </div>
    ...
    def run(self):
        ...
        # self.syncVisuleReport()


if __name__ == "__main__":
    parser = TestReportGenerator(trigger_id=12345, agent_id="agent15dut1")
